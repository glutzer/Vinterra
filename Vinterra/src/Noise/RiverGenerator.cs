...
public class RiverGenerator
{
    ...
    {
        riverDepth = VConfig.Loaded.riverDepth;
        baseDepth = VConfig.Loaded.baseDepth;
    ...
    public RiverSample SampleRiver(List<RiverSegment> segments, double x, double z)
    {
        ...
        {
            float riverProjection = VMath.GetProjection(point, segment.river.startPoint, segment.river.endPoint); //How far along the main river for size calculation

            double riverSize = GameMath.Lerp(segment.river.startSize, segment.river.endSize, riverProjection); //Size of river

            float segmentProjection = VMath.GetProjection(point, segment.startPoint, segment.endPoint);
            ...
            foreach (RiverSegment connector in connectors)
            {
                ...
                {
                    midPointProjection = VMath.GetProjection(point, segment.midPoint, connector.midPoint);
                }
                else
                {
                ...
                double distance = VMath.DistanceToLine(point, lerpedStart, lerpedEnd);
                //double distance = VMath.DistanceToLine(point, segment.startPoint, segment.endPoint);
                ...
                if (distance <= riverSize) //If within bank distance
                {
                    if (distance < closestLine)
                    {
                        ...
                        {
                            riverSample.flowVectorX = (float)segmentFlowVector.X * segment.river.speed; //10 is speed
                    ...
                    double lerp = 1 - VMath.InverseLerp(distance, riverSize, 0);
